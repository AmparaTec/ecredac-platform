  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-500 mt-1">Gerencie seus convites de indicação</p>
      </div>

      {/* Referral Code */}
      <Card className="p-6 bg-gradient-to-r from-brand-500/10 to-purple-500/10 border-dark-500/50">
        <h2 className="text-sm font-bold text-brand-400 mb-2">Seu Código de Indicação</h2>
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 px-4 py-3 bg-dark-700 rounded-xl border-2 border-dashed border-brand-500/50">
            <span className="font-mono text-2xl font-black tracking-[0.3em] text-brand-400">
              {profile.referral_code || '--------'}
            </span>
          </div>
        </div>
        <p className="text-xs text-slate-500 mt-3">
          Link: <span className="font-mono text-brand-400">ecredac.com.br/register?ref={profile.referral_code || '...'}</span>
        </p>
      </Card>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card className="p-5">
          ...
          <p className="text-2xl font-bold mt-1 text-white">{allInvites.length}</p>
        </Card>
        <Card className="p-5">
          <p className="text-sm text-slate-500">Aceitos</p>
          <p className="text-2xl font-bold mt-1 text-emerald-400">{accepted}</p>
        </Card>
        <Card className="p-5">
          <p className="text-sm text-slate-500">Pendentes</p>
          <p className="text-2xl font-bold mt-1 text-amber-400">{pending}</p>
        </Card>
      </div>
        ...
        {allInvites.length > 0 ? (
          <div className="space-y-3">
            {allInvites.map((invite: any) => {
              ...
              return (
                <div key={invite.id} className="flex items-center justify-between p-4 rounded-xl bg-dark-600/50 hover:bg-dark-600 transition-all">
                  <div className="flex items-center gap-3">
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${
                      invite.status === 'accepted' ? 'bg-emerald-500/15 text-emerald-400' :
                      invite.status === 'pending' ? 'bg-amber-500/15 text-amber-400' :
