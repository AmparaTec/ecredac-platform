  ...
  if (filtered.length === 0) {
    return (
      <Card className="p-5 text-center">
        ...
        <p className="text-sm text-gray-500">Nenhum benchmark disponivel</p>
        <p className="text-xs text-gray-400">Dados de mercado serao gerados conforme transacoes ocorrerem</p>
      </Card>
    )
  }
  ...
  if (compact) {
    return (
      <div className="flex items-center gap-3 p-3 rounded-xl bg-gray-50 border border-gray-200">
        <BarChart3 size={16} className="text-gray-500 flex-shrink-0" />
        <div className="flex-1 flex items-center gap-4 text-xs">
          <span className="text-gray-500">
            Media mercado: <strong className="text-gray-900">{formatDiscount(generalBenchmark?.avg_discount || 0)}</strong>
          </span>
          <span className="text-gray-500">
          ...
          {generalBenchmark?.discount_trend !== 0 && (
            <span className={`flex items-center gap-0.5 font-medium ${
              (generalBenchmark?.discount_trend || 0) < 0 ? 'text-emerald-600' : 'text-red-600'
            }`}>
      ...
      <div className="p-5 pb-3">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="text-sm font-bold text-gray-900">Benchmark de Mercado</h3>
        ...
        <div className="flex gap-1.5 mb-3">
          <button
            ...
            className={`px-2.5 py-1 rounded-lg text-xs font-medium transition-all ${
              !selectedGrade ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
            }`}
          >
            Todos
          </button>
          {(['A', 'B', 'C', 'D'] as CreditScoreGrade[]).map(grade => {
            const cfg = creditScoreConfig[grade]
        ...
        <div className="space-y-2">
          {(selectedGrade ? gradeBenchmarks.filter(b => b.credit_grade === selectedGrade) : gradeBenchmarks.length > 0 ? gradeBenchmarks : [generalBenchmark]).filter(Boolean).map((bm) => {
            if (!bm) return null
            const gradeConfig = bm.credit_grade ? creditScoreConfig[bm.credit_grade] : null
            return (
              <div key={bm.id} className="p-3 rounded-xl bg-gray-50 border border-gray-100">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    {gradeConfig ? (
                      ...
                      <span className="text-xs font-bold text-gray-500">Geral</span>
                    )}
                    <span className="text-xs text-gray-400">{bm.transaction_count} transacoes</span>
                  </div>
                  <div className="flex items-center gap-2">
                    {bm.discount_trend !== 0 && (
                      <span className={`flex items-center gap-0.5 text-xs font-medium ${
                        bm.discount_trend < 0 ? 'text-emerald-600' : 'text-red-600'
                      }`}>
                ...
                <div className="grid grid-cols-5 gap-2 text-xs">
                  <div>
                    ...
                    <p className="font-bold text-gray-900">{formatDiscount(bm.avg_discount)}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    <p className="text-gray-400 flex items-center gap-0.5"><Clock size={10} /> Dias p/ vender</p>
                    <p className="font-medium">{bm.avg_days_to_sell ? `${bm.avg_days_to_sell.toFixed(0)} dias` : '—'}</p>
                  </div>
                  <div>
                    <p className="text-gray-400 flex items-center gap-0.5"><Droplets size={10} /> Liquidez</p>
